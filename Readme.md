<!-- =====================================================
     SPACE THEME + TERMINAL ANIMATION
     GitHub Profile README — Ujwal Dimri
     ===================================================== -->

<h1 align="center">🚀 Ujwal Dimri</h1>

<p align="center">
  <i>B.Tech CSE (2nd Year) • UPES Dehradun</i><br/>
  <b>Exploring the Tech Universe</b> 🌌
</p>

---

<!-- ===== SPACE TERMINAL ANIMATION (API ENABLED) ===== -->
<p align="center">
  <img
    src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=16&duration=2200&pause=800&color=36BCF7&center=true&vCenter=true&width=620&lines=space@orbit:~$+whoami;ujwal_dimri;space@orbit:~$+current_mission;learning_devops_and_web;space@orbit:~$+status;in_orbit_%F0%9F%8C%8C"
    alt="Space Terminal Animation"
  />
</p>

---

## 🪐 About Me

I am a **first-year Computer Science student** at **UPES, Dehradun**, navigating through the vast universe of technology.

I enjoy working close to systems, exploring **Linux**, and building strong fundamentals in **programming and web development**, with a long-term goal of becoming a **DevOps Engineer**.

I believe growth is orbital — slow, steady, and unstoppable.

---

## 🌠 Skills

- **Programming:** C  , Python 
- **Web:** HTML, CSS  , JavaScript ,Node.js , Express.js
- **Systems:** Linux
- **Backend** Node.js , Express.js
- **DataBase** SQL,MongoDB
- **API-TestingTools** PostMan , Hoppscotch 
- **Interests:** DevOps, Cloud, Open Source
   

---

## 🛰️ Education

**B.Tech – Computer Science & Engineering**  
University of Petroleum and Energy Studies (UPES)  
*2025 – 2029*

---

## 🌌 Mission Log

- 🚀 Build strong system foundations  
- 🛠️ Learn DevOps & cloud tools  
- 🌍 Contribute to open source  
- 📈 Improve every single day  

---

## 📡 Contact Station

- 📧 **Email:** ujwaldimri223@gmail.com  
- 🎓 **Student Email:** Ujwal.22777@stu.upes.ac.in  
- 🔗 **LinkedIn:**  
  https://www.linkedin.com/in/ujwal-dimri-82400729a/  
- 💻 **GitHub:**  
  https://github.com/UjwalDimri  

---

```text
🌌 space-terminal@orbit:~$ echo "keep learning"
keep learning
